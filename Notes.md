...
## AI source analysis (first release)

- After PDF/TXT extraction, owners/editors can request a separate AI job. It samples up to 16 page-labelled passages (1,200 characters each), asks for a short summary and up to five findings, and shows cited excerpts beside the output. Viewers can read results but not start paid work.
- OpenAI Responses Structured Outputs enforce the response shape; the server rejects unknown passage IDs and incomplete output. This checks citation provenance, **not** whether a cited passage truly proves a claim. Users should verify claims against the source.
- The API key stays server-side; `OPENAI_API_KEY` is required and `OPENAI_MODEL` is configurable. The UI asks for confirmation before sending private text externally; requests use `store: false`. Teams still need a privacy/retention review before production use.
